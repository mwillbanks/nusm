---
title: Quick Start
description: Install nusm, create a store, choose its lifetime, and update it safely.
icon: Rocket
---
...
export const counterStore = createNusmStore({ count: 0 })
```

It uses the same `state`, `setState`, `subscribe`, React, and Devtools APIs as a persisted store. Its `ready` promise resolves immediately, `hydration.overall` is `not_configured`, and its state resets when the runtime reloads.
