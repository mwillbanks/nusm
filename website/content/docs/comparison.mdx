...
Using nusm for transient state does not write that state anywhere. It keeps one store type across the application while leaving persistence as a per-store choice.
