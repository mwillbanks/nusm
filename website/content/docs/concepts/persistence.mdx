...
Persistence is opt-in and belongs to each store, not to the application as a whole. Omit both `adapter` and `persist` for in-memory state. Add an adapter and stable `storeId` only when that store must cross a storage boundary.
